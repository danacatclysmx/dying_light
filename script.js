...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateImagePosition() {
        ...
        if (imagePosition < windowHeight * 0.9 && imagePosition > -100) { // Ajustar umbrales para que desaparezca más tarde
            const translateX = Math.max(0, initialOffset - (windowHeight * 0.9 - imagePosition) * 0.3); // Movimiento más gradual
            craneImage.style.transform = `translateX(${translateX}px)`;
            craneImage.style.opacity = "1";
        } else if (imagePosition <= -150) { // Ajustar punto de desaparición más abajo
    ...
    craneImage.style.webkitMask = "url(imgs/mask_head.png) no-repeat 80% 100% / 0 0";
    craneImage.style.mask = "url(imgs/mask_head.png) no-repeat 80% 100% / 0 0";
    craneImage.style.transition += ", mask-size 1.5s ease-in, -webkit-mask-size 1.5s ease-in";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateImagePosition() {
        ...
        if (imagePosition < windowHeight * 0.9 && imagePosition > -100) {
            const translateX = Math.max(0, initialOffset - (windowHeight * 0.9 - imagePosition) * 0.3);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    formulario.addEventListener("submit", function (event) {
      ...
      // Validación básica
      if (nombre.trim() === "" || tipo.trim() === "" || descripcion.trim() === "") {
